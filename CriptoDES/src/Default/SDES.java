...
public class SDES {
	...
	public static int permuta(int x, int p[], int pmax) {
		int y = 0;
	...
	public byte encripta(int m)

	{
		System.out.println("\nInicio do processo de encriptação........\n\n");
		m = permuta(m, IP, IPmax);
		System.out.print("\nIP: ");
		printaDado(m, 8);
		m = fK(m, K1);
		System.out.print("\nAFK: ");
		...
		System.out.print("\nFK: ");
		printaDado(m, 8);
		m = permuta(m, IPI, IPImax);
		return (byte) m;

	}

	public byte decripta(int m)
	...
	public static void printaDado(int x, int n) {
		int mask = 1 << (n - 1);
		while (mask > 0) {
